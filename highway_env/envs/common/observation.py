...
class GrayscaleObservation(ObservationType):
    ...
    def __init__(self, env: 'AbstractEnv',
                 observation_shape: Tuple[int, int] = (64,128), #H xW
                 stack_size: int = 4,
        ...
        viewer_config.update({
            ...
            "screen_width": self.observation_shape[1]*4,
            "screen_height": self.observation_shape[0]*4,
    ...
    def observe(self) -> np.ndarray:
        ...
        # img = Image.fromarray(new_obs).show()
        new_obs = resize(new_obs, output_shape=self.observation_shape,anti_aliasing=True)
        # img = Image.fromarray(new_obs*255).show()
        # new_obs = new_obs[::4, ::4]
        # img = Image.fromarray(new_obs).show()
        # new_obs = new_obs / 255
...
class GrayscaleObservationLatentSpace(ObservationType):
    ...
    def __init__(self, env: 'AbstractEnv',
        ...
        self.latent_dimention = self.env.config['latent_dimention']
        # Compute latent space
        autoencoder_path = self.env.config['project_root']
        # MLPmodel = "C:/Users/rvali/PycharmProjects/rl-agents/Autoencoder/MLP"
        Imagemodel = os.path.join(autoencoder_path, 'Autoencoder/Image' + str(self.latent_dimention))
        self.autoencoderImage = Autoencoder.load(Imagemodel)

    def space(self) -> spaces.Space:
        return spaces.Box(shape=(self.latent_dimention, 1), low=-np.inf, high=np.inf, dtype=np.float32)
    ...
    def observe(self) -> np.ndarray:
        ...
        # Compute latent space , convert to H,W,C input to keras
        # state = np.moveaxis(new_obs, 0, -1)
        state = np.expand_dims(new_obs, axis=0)
        reconstructed_obs, latent_representations = self.autoencoderImage.reconstruct(state)

        obs = latent_representations.reshape(self.latent_dimention, 1)
...
class ImageObservation(ObservationType):
    """
    An observation from render image
    ...
    Example of observation dictionary in the environment config:
        observation": {
            ...
            "weights": [0.2989, 0.5870, 0.1140],  #weights for RGB conversion,
            "stack_size": 4,
            "observation_shape": [400, 120]
            "perception_horizon": [10, 50],  [behind, front]
        }
    """
    ...
    def observe(self) -> np.ndarray:
        ...
        new_obs = resize(new_obs, self.observation_shape)
        # img = Image.fromarray(new_obs).show()
    ...
    def _record_to_grayscale(self) -> np.ndarray:
        raw_rgb = self.env.render(mode='rgb_array')
        # img = Image.fromarray(raw_rgb).show()
        surface = self.env.viewer.sim_surface
        observer_position_px = surface.vec2pix(self.observer_vehicle.position)
        distance_to_pixel_front = surface.pix(self.perception_horizon[1])
        distance_to_pixel_behind = surface.pix(self.perception_horizon[0])
        # print("shape " + str(np.shape(raw_rgb)) )
        # print("observer position :" + str(observer_position_px))
        # TODO: add padding
        shapes = np.shape(raw_rgb)
        up = int(shapes[0] *0.4)
        down = int(shapes[0] * 0.75)
        img_crop = raw_rgb[up:down,
                   observer_position_px[0] - distance_to_pixel_behind:observer_position_px[0] + distance_to_pixel_front,
                   :]
        # img = Image.fromarray(img_crop).show()
        # img_gray = color.rgb2gray(img_crop)
        img_gray = np.dot(img_crop[..., :3], self.config['weights'])
        # img = Image.fromarray(img_gray).show()
        # TODO: fix gray scale
        # print("shape_gray_image " + str(np.shape(img_gray)))
        img_gray_shape = np.shape(img_gray)
        if img_gray_shape[0] <= 5 or img_gray_shape[1] <=5:
            img_gray = np.zeros(self.observation_shape)
            # print("*********************************************shape_gray_image_zeros " + str(np.shape(img_gray)))
        return img_gray
...
class KinematicObservationLatentSpace(ObservationType):
    ...
    def __init__(self, env: 'AbstractEnv',
        ...
        autoencoder_path = self.env.config['project_root']
        # MLPmodel = "C:/Users/rvali/PycharmProjects/rl-agents/Autoencoder/MLP"
        MLPmodel = os.path.join(autoencoder_path,'Autoencoder/MLP'+str(self.latent_dimention))
        self.autoencoderMLP = DeepAutoencoder.load(MLPmodel)
    ...
    def normalize_obs(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.features_range:
            ...
            self.features_range = {
                "x": [-2.0 * MDPVehicle.SPEED_MAX, 2.0 * MDPVehicle.SPEED_MAX],
    ...
    def observe(self) -> np.ndarray:
        ...
        flat_obs = np.array(flat_obs).reshape(1,42)
        reconstructed_obs, latent_representations = self.autoencoderMLP.reconstruct(flat_obs)
...
class OccupancyGridObservationLatentSpace(ObservationType):
    ...
    def __init__(self,
                 ...
                 align_to_vehicle_axes: bool = False,
                 clip: bool = True,
                 as_image: bool = False,   latent_dimention: int = 16,
                 **kwargs: dict) -> None:
        """
        :param env: The environment to observe
        :param features: Names of features used in the observation
        ...
        autoencoder_path = self.env.config['project_root']
        # MLPmodel = "C:/Users/rvali/PycharmProjects/rl-agents/Autoencoder/MLP"
        Gridmodel = os.path.join(autoencoder_path, 'Autoencoder/Grid' + str(self.latent_dimention))
        self.autoencoderGrid= Autoencoder.load(Gridmodel)
    ...
    def observe(self) -> np.ndarray:
        ...
        if self.absolute:
            ...
            # Compute latent space , convert to H,W,C input to keras
            state = np.moveaxis(obs, 0, -1)
            state =  np.expand_dims(state, axis=0)
            reconstructed_obs, latent_representations = self.autoencoderGrid.reconstruct(state)
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    ...
    elif config["type"] == "KinematicObservationLatentSpace":
        return KinematicObservationLatentSpace(env, **config)
    elif config["type"] == "OccupancyGrid":
        return OccupancyGridObservation(env, **config)
    elif config["type"] == "OccupancyGridObservationLatentSpace":
        return OccupancyGridObservationLatentSpace(env, **config)
    ...
    elif config["type"] == "GrayscaleObservationLatentSpace":
        return GrayscaleObservationLatentSpace(env, **config)
